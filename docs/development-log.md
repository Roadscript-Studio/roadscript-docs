...
3. `cyphra-cli` was established as the standalone public repository for Cyphra CLI.
4. Cyphra CLI was updated to build against the installed Engine package instead of directly including Engine source files.
5. The Engine stopped owning `apps/cli` and `apps/tui`.
6. The Engine became core-only and remained private.
7. Cyphra CLI became application- and tooling-focused and was prepared for public release.
8. The Cyphra CLI public release was sanitized to remove local paths, sensitive data, private source references, and build artifacts.
9. Public examples were retained in Cyphra CLI, including:
...
- Keep `cyphra-cli` reviewable through examples, tests, and tooling
